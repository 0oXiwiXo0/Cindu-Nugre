...
final class PhotoController extends AbstractController
{
    ...
    public function create(Request $request, EntityManagerInterface $em): Response
    {
        ...
        if ($photoForm->isSubmitted() && $photoForm->isValid()){
            ...
            $this->addFlash('success', 'Photo successfully added!');
           ...
           EntityManagerInterface $em): Response
    {
        ...
        if ($photoForm->isSubmitted() && $photoForm->isValid()) {
            // hydrate les propriétés absentes du formulaire
            $photo->setDateUpdated(new \DateTime());
            // sauvegarde en bdd
            $em->flush();
            // affiche un message sur la prochaine page
    ...
    public function delete(int $id, PhotoRepository $photoRepository, Request $request, EntityManagerInterface $em): Response
    {
        $photo = $photoRepository->find($id);
        if (!$photo) {
            throw $this->createNotFoundException('Cette photo n\'existe pas');
